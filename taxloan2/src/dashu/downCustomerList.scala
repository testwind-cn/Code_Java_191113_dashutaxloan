...
object downCustomerList {
  def run(hc:HiveContext): Unit = {
    ...
    val down_customer_list_tmp1_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.down_customer_list_tmp1))
    ...
    val down_customer_list_tmp2_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.down_customer_list_tmp2))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    val down_customer_list_tmp3_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.down_customer_list_tmp3))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    val down_customer_list_tmp4_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.down_customer_list_tmp4))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    //  根据日期维度表生成季度维度表
    val dim_quarter_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.dim_quarter))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    dim_quarter_df.write.format("orc").mode("overwrite").saveAsTable("dim_quarter")
    println("dim_quarter 计算完成")

    //创建卖方税号、季度维度表
    val dim_sellertaxno_quarter_24month_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.dim_sellertaxno_quarter_24month))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    val down_customer_list_df:DataFrame = hc.sql(spark.printCmd(constant_full.downCustomerList.down_customer_list))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
