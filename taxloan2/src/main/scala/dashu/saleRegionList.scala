...
//    val hc:HiveContext = new HiveContext(sc)
    ...
    val sale_region_list_tmp1_df:DataFrame = hc.sql(sparkTool.printCmd(constant_full.saleRegionList.sale_region_list_tmp1))
    ...
    //  创建近2年维度表
    val dim_sellertaxno_latest_2_years_df:DataFrame = hc.sql(sparkTool.printCmd(constant_full.saleRegionList.dim_sellertaxno_latest_2_years))
    // hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    val sale_region_list_tmp2_df:DataFrame = hc.sql(sparkTool.printCmd(constant_full.saleRegionList.sale_region_list_tmp2))
    // hc.sql(s"use ${constant_common.Const_common.DATABASE}")
    ...
    val sale_region_list_df:DataFrame = hc.sql(sparkTool.printCmd(constant_full.saleRegionList.sale_region_list))
    //hc.sql(s"use ${constant_common.Const_common.DATABASE}")
