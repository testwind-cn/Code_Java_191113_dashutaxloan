...
select
    ...
    (
        CASE
            WHEN t2.invoice_red_cnt=0 AND t2.invoice_cnt=0 THEN -9997
            WHEN t2.invoice_red_cnt!=0 AND t2.invoice_cnt=0 THEN -9996
            ELSE t2.invoice_red_cnt/t2.invoice_cnt
        END
    ) AS invoice_red_cnt_rate, -- 红冲发票张数占比

    (
        CASE
            WHEN t2.invoice_cancel_cnt=0 AND t2.invoice_cnt=0 THEN -9997
            WHEN t2.invoice_cancel_cnt!=0 AND t2.invoice_cnt=0 THEN -9996
            ELSE t2.invoice_cancel_cnt/t2.invoice_cnt
        END
    ) AS invoice_cancel_cnt_rate, -- 作废发票张数占比

    (
        CASE
            WHEN t2.invoice_total_r_sum=0 AND t2.invoice_total_sum=0 THEN -9997
            WHEN t2.invoice_total_r_sum!=0 AND t2.invoice_total_sum=0 THEN -9996
            ELSE t2.invoice_total_r_sum/t2.invoice_total_sum
        END
    ) AS invoice_red_total_sum_rate, -- 红冲发票合计金额税额总和比率

    (
        CASE
            WHEN t2.invoice_total_c_sum=0 AND t2.invoice_total_sum=0 THEN -9997
            WHEN t2.invoice_total_c_sum!=0 AND t2.invoice_total_sum=0 THEN -9996
            ELSE t2.invoice_total_c_sum/t2.invoice_total_sum
        END
    ) AS invoice_cancel_total_sum_rate, -- 作废发票合计金额税金总和比率
...
left join t2
on
    t2.mcht_cd=t5.mcht_cd and t2.data_month=t5.data_month
left join t3
on
    t2.mcht_cd=t3.sellertaxno and t2.data_month=t3.data_month
left join t4
on
    t3.sellertaxno=t4.sellertaxno and t3.data_month=t4.data_month
left join t6
on
    t6.sellertaxno=t4.sellertaxno and t6.data_month=t4.data_month
